...
const RUNNER = path.resolve("scripts/run-automation.ps1");
const dirs: string[] = [];
afterEach(() => { for (const dir of dirs.splice(0)) fs.rmSync(dir, { recursive: true, force: true }); });
function tempDir(): string { const dir = fs.mkdtempSync(path.join(os.tmpdir(), "s2l-runner-")); dirs.push(dir); return dir; }
...
function writeFakeNpm(dir: string, exitCode: number): void {
  const lines = ["@echo off", "echo npm args: %*", "echo collector warning 1>&2", `exit /b ${exitCode}`];
  fs.writeFileSync(path.join(dir, "npm.cmd"), `${lines.join("\r\n")}\r\n`, "utf8");
}
...
async function runRunner(npmExitCode: number, configPath: string): Promise<{ code: number; log: string }> {
  ...
  const args = ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", RUNNER, "-ConfigPath", configPath, "-LogPath", logPath];
  let code = 0;
  try { await execFileAsync("powershell.exe", args, { env: envWithFakeNpm(binDir) }); }
  catch (error) { code = (error as { code?: number }).code ?? -1; }
  return { code, log: fs.existsSync(logPath) ? fs.readFileSync(logPath, "utf8") : "" };
}

describe.skipIf(process.platform !== "win32")("scheduler runner", () => {
  it("reports success to Task Scheduler when npm exits 0 after writing to stderr", async () => {
  ...
  it("propagates a failed npm exit code to Task Scheduler", async () => {
