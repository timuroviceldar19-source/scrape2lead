...
describe("getJobTelemetry", () => {
  ...
  beforeEach(() => {
    ...
    parseJobId = storage.createParseJob({ source: "2gis", city: "moscow", category: "test" });
  });
  afterEach(() => cleanup());

  it("returns zero rates and null timings for a job with only pending tasks", () => {
    storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: "ext-1" });
    const t = storage.getJobTelemetry(parseJobId);
  ...
  it("computes task rates from mixed terminal statuses", () => {
    const ids: number[] = [];
    for (let i = 0; i < 5; i++) {
      ids.push(storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: `ext-${i}` }));
    }
  ...
  it("counts captcha events only from this job's tasks", () => {
    const taskId = storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: "ext-1" });

    const otherJobId = storage.createParseJob({ source: "2gis", city: "spb", category: "test" });
    const otherTaskId = storage.enqueueCompanyTask({ parseJobId: otherJobId, source: "2gis", externalId: "ext-x" });

    storage.saveCaptchaEvent({ source: "2gis", action: "detected", companyTaskId: taskId });
    storage.saveCaptchaEvent({ source: "2gis", action: "rotated",  companyTaskId: taskId });
    // Other job's event — must not be counted.
    storage.saveCaptchaEvent({ source: "2gis", action: "detected", companyTaskId: otherTaskId });

    const t = storage.getJobTelemetry(parseJobId);
    expect(t.captcha_count).toBe(2);
  });

  it("ban_count covers result='blocked' and error_type='blocked', not plain failures", () => {
    const taskId = storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: "ext-1" });
    // result='blocked' → counted
    storage.saveAttempt({ source: "2gis", externalId: "ext-1", status: "blocked", companyTaskId: taskId });
    // error_type='blocked' with a different result → also counted
    storage.saveAttempt({ source: "2gis", externalId: "ext-1", status: "error", errorType: "blocked", companyTaskId: taskId });
    // plain failure — must not be counted
    storage.saveAttempt({ source: "2gis", externalId: "ext-1", status: "failed", companyTaskId: taskId });
    // success — must not be counted
    storage.saveAttempt({ source: "2gis", externalId: "ext-1", status: "success", companyTaskId: taskId });

    const t = storage.getJobTelemetry(parseJobId);
    expect(t.ban_count).toBe(2);
  });

  it("counts proxy rotations within the job time window only", () => {
  ...
  it("computes average_parse_time from non-null durations only", () => {
    const taskId = storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: "ext-1" });
    storage.saveAttempt({ source: "2gis", externalId: "ext-1", status: "success", companyTaskId: taskId, durationMs: 100 });
    storage.saveAttempt({ source: "2gis", externalId: "ext-1", status: "error",   companyTaskId: taskId, durationMs: 200 });
  ...
  it("returns null average_parse_time when no attempts carry a duration", () => {
    const taskId = storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: "ext-1" });
    storage.saveAttempt({ source: "2gis", externalId: "ext-1", status: "failed", companyTaskId: taskId });
  ...
  it("computes cards_per_hour from total task count and elapsed hours", () => {
    const start = "2026-06-01T10:00:00.000Z";
    ...
    for (let i = 0; i < 4; i++) {
      storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: `ext-${i}` });
    }
  ...
  it("returns null cards_per_hour when start == end (zero elapsed time)", () => {
  ...
  it("isolates all metrics per job — another job's data does not bleed in", () => {
    const taskId = storage.enqueueCompanyTask({ parseJobId, source: "2gis", externalId: "ext-1" });
    ...
    storage.saveAttempt({ source: "2gis", status: "blocked", companyTaskId: otherTaskId });
    storage.saveCaptchaEvent({ source: "2gis", action: "detected", companyTaskId: otherTaskId });
  ...
  it("handles an unknown parseJobId gracefully — all zeros / nulls", () => {
    const t = storage.getJobTelemetry("nonexistent-job-id");
    expect(t.terminal_count).toBe(0);
    expect(t.success_rate).toBe(0);
    expect(t.captcha_count).toBe(0);
    expect(t.ban_count).toBe(0);
