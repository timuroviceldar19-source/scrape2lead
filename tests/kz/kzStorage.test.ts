...
describe("KzStorage", () => {
  ...
  it("does not treat contactless registry rows as fresh when contacts are required", () => {
    const db = new Database(":memory:");
    const storage = new KzStorage({ db });
    const record: GoszakupRegistryRecord = {
      bin: "123456789012",
      participant_id: "777",
      name_ru: "No Contacts",
      name_kz: null,
    ...
    storage.upsertGoszakupRegistry(record);

    expect(storage.isGoszakupRegistryFresh("123456789012", 7, new Date("2026-06-30T00:00:00.000Z"))).toBe(true);
    expect(storage.isGoszakupRegistryFresh("123456789012", 7, new Date("2026-06-30T00:00:00.000Z"), { requireAnyContact: true })).toBe(false);

    storage.upsertGoszakupRegistry({ ...record, email: "contact@example.kz" });
    expect(storage.isGoszakupRegistryFresh("123456789012", 7, new Date("2026-06-30T00:00:00.000Z"), { requireAnyContact: true })).toBe(true);
    db.close();
  });
