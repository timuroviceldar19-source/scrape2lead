...
describe("ProcurementStorage", () => {
  ...
  it("round-trips optional plan detail JSON", () => {
    const storage = new ProcurementStorage({ db: new Database(":memory:") });
    storage.upsert(row({ planDetail: { approvedAt: "2026-04-15", financialYear: 2026, planYearId: 12, planMonth: null, nameRu: "Ноутбук",
    ...
    expect(storage.list()[0]?.planDetail).toMatchObject({ financialYear: 2026, planYearId: 12, planMonth: null, quantity: 4,
