...
[`src/kz/channelDigest.ts`](../src/kz/channelDigest.ts) — форматирует **публичный пост** для Telegram-канала из списка winners (укороченные названия, суммы, CTA на WhatsApp). Это маркетинговая обёртка над теми же данными autopilot, не отдельный сбор.
