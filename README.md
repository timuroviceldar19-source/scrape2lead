...
Второй cron у каждой задачи — backstop. Он проверяет через API, был ли сегодня
успешный прогон этого же workflow, и пропускает себя, если был; фактическая
работа выполняется один раз в день.
