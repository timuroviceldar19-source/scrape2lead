...
Guard пропускает повтор **только для `event=schedule`**. Ручной запуск и внешний
`workflow_dispatch` выполняются безусловно — на этом и держится дневная пара слотов
13:00/14:30 (см. ниже): это не backstop, а осознанный повторный сбор в тот же день.
...
публичного HTTP endpoint и вызывается только через пять Cron Trigger:
...
`UF_CRM_PLAN_ID`, уже заведённый план второй сделки не создаёт. Порядок цифр за
27.07.2026: из 346 строк утреннего прогона `create=1`, `existing=299`, `duplicate=46`.

Сторож стоит **между** парами, а не после дневной: иначе дневной прогон закрывал бы
пропуск утреннего раньше, чем о нём стало бы известно. Обратная сторона — зазор после
`gz-daily-main.yml` сжался с трёх часов до полутора, разбор в шапке `gz-watchdog.yml`.
...
4. Открыть поток боевых логов:
   ...
   **Идентификатора run в логе не будет — это норма.** Замерено 2026-07-26: GitHub
   отвечает на `/dispatches` кодом `204 No Content` с пустым телом, поэтому в записи
   есть `status: 204`, но нет `workflowRunId`. Связать вызов Worker'а с конкретным
   прогоном нужно по времени и по полю `workflow`. Обработка `2xx` с телом оставлена
   на случай, если GitHub когда-нибудь начнёт возвращать идентификатор.

   Изменения Cron Trigger могут распространяться в Cloudflare до 15 минут, поэтому
   первый замер делают после этого окна.
