...
export class KzStorage {
  ...
  isGoszakupRegistryFresh(
    ...
    const row = this.db.prepare(
      "SELECT updated_at, phone, email, website FROM goszakup_registry_data WHERE bin = ?"
  ...
  upsertGoszakupRegistry(record: GoszakupRegistryRecord): void {
    this.db.prepare(`
      ...
      ON CONFLICT(bin) DO UPDATE SET
        ...
        full_address_ru = excluded.full_address_ru,
        reporting_administrator = excluded.reporting_administrator,
